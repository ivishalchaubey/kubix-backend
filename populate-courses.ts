...
import { Course, ICourse } from './src/app/modules/courses/models/Course';
import User from './src/app/modules/auth/models/User';
import Category from './src/app/modules/admin/categories/models/Category';
import { config } from './src/app/config/env';

// Interface for course data from JSON
...
async function findUniversityByName(universityName: string): Promise<mongoose.Types.ObjectId | null> {
  try {
...
async function findCategoryByName(categoryName: string): Promise<mongoose.Types.ObjectId | null> {
  try {
...
async function findParentCategory(categoryId: mongoose.Types.ObjectId): Promise<mongoose.Types.ObjectId | null> {
  try {
...
async function populateCourses(): Promise<void> {
  try {
    ...
    const coursesData: CourseData[] = JSON.parse(fs.readFileSync(coursesDataPath, 'utf8'));
    ...
    for (const courseData of coursesData) {
      try {
        ...
        const courseObject: Partial<ICourse> = {
        ...
        console.error(`❌ Error processing course ${courseData.name}:`, (error as Error).message);
        errorCount++;
      }
    }
...
async function createSampleUniversities(): Promise<void> {
  const universities = [
    {
      firstName: "IIT",
      lastName: "Delhi",
  ...
  for (const university of universities) {
    try {
      ...
      console.error(`❌ Error creating university ${university.collegeName}:`, (error as Error).message);
    }
  }
}

// Function to create sample categories if they don't exist
async function createSampleCategories(): Promise<void> {
  const categories = [
    {

...
class ImageUploadController {
    ...
    // Upload to S3
    let result = await uploadFile(req.file.originalname, req.file.buffer , req.file.mimetype);

    ResponseUtil.created(res, {data : result}, API_MESSAGES.SUCCESS.AWS_FILE_UPLOADED);
        } catch (error) {
            next(error);
        }
    };
